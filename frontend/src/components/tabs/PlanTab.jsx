  ...
  return (
    <div className="animate-fade-in-up">
      <div className="flex justify-between items-center mb-6">
        <h3 className="text-xl font-bold text-bajet-cream">Saving Plans</h3>
        <button 
      ...
      {savingPlans.length === 0 ? (
        <div className="bg-[#3a3a3a] p-12 rounded-xl shadow-sm border border-[#4a4a4a] flex flex-col items-center justify-center text-center">
          ...
          <h3 className="text-xl font-bold text-bajet-cream mb-2">Start Saving Today</h3>
          <p className="text-gray-400 max-w-sm">Create a saving plan for a vacation, a new car, or an emergency fund.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {savingPlans.map(plan => {
            ...
            return (
              <div key={plan.id} className="bg-[#3a3a3a] p-6 rounded-xl shadow-sm border border-[#4a4a4a] hover:border-bajet-purple transition-all group">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h4 className="text-lg font-bold text-bajet-cream">{plan.name}</h4>
                    <p className="text-sm text-gray-400">{plan.contributions?.length || 0} contributions</p>
                  </div>
                  <button 
                    onClick={() => { setSelectedPlan(plan); setIsFundsModalOpen(true); }}
                    className="text-sm px-3 py-1.5 bg-[#2f2f2f] text-[#a3e635] hover:bg-[#a3e635] hover:text-[#2f2f2f] font-medium rounded-md border border-[#4a4a4a] transition-all"
                  >
                    + Add Funds
                  </button>
                </div>
                ...
                <div className="mb-2 flex justify-between items-end">
                  <span className="text-2xl font-bold text-[#a3e635]">RM {currentAmount.toFixed(2)}</span>
                  <span className="text-sm font-medium text-gray-400">/ RM {targetAmount.toFixed(2)}</span>
                </div>
                
                <div className="w-full bg-[#2f2f2f] rounded-full h-3 mb-2 overflow-hidden border border-[#4a4a4a]">
                ...
                {plan.contributions?.length > 0 && (
                  <div className="mt-4 pt-4 border-t border-[#4a4a4a]">
                    <h5 className="text-xs font-semibold text-gray-400 uppercase mb-2">Recent History</h5>
                    <div className="space-y-2 max-h-32 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-[#4a4a4a]">
                      {[...plan.contributions].sort((a,b) => new Date(b.date) - new Date(a.date)).slice(0, 5).map(c => (
                        <div key={c.id} className="flex justify-between items-center text-sm">
                          <div>
                            <span className="text-gray-300 block">{c.description || 'Added funds'}</span>
                            <span className="text-xs text-gray-500">{c.date}</span>
                          </div>
                          <span className="text-[#a3e635] font-medium">+RM {parseFloat(c.amount).toFixed(2)}</span>
                        </div>
                      ))}
                    </div>
                  </div>
